...
const StudentDashboard: React.FC = () => {
  ...
  // Surface fetch errors from a top-level effect so hooks remain stable
  React.useEffect(() => {
    if (error) {
      toast({
        title: 'Error',
  ...
  const upcomingClasses = dashboardData?.data?.upcomingClasses?.slice(0, 3) || [];
  const batches = dashboardData?.data?.batches || [];
  const user = dashboardData?.data?.user;

  // Transform schedule data to include batch information
  const scheduleWithBatchInfo =
    dashboardData?.data?.schedule?.map((item: any, index: number) => ({
      ...item,
      batchId: batches[index]?._id || `batch-${index}`,
      courseTitle: batches[index]?.course?.title || 'Course',
    })) || [];

  const upcomingClassesWithInfo = scheduleWithBatchInfo.slice(0, 3);

  const totalBatches = batches.length;
  const completedBatches = batches.filter((batch: Batch) =>
    batch.schedule.some((s: any) => s.endTime < new Date())
  ).length;

  const progressPercentage = totalBatches > 0 ? (completedBatches / totalBatches) * 100 : 0;

  // Calculate additional stats (mock for now - to be replaced with real API data)
  const attendancePercentage = 85;
  const recentTestScore = 92;
  const notificationsCount = 5;
  const studyStreak = user?.gamification?.streak?.current || 7;
  ...
  if (error) {
    // Error UI shown below; toast is handled by top-level effect
    return (
      <Layout>
                  ...
                  <div className='flex items-center justify-between'>
                    <div className='flex items-center gap-3'>
                      ...
                      <div>
                        ...
                        <p className='text-sm text-gray-600 mt-1'>Your next scheduled learning sessions</p>
                      </div>
                    </div>
                    <Badge className='bg-blue-100 text-blue-700'>{upcomingClasses.length} Classes</Badge>
                  </div>
                </CardHeader>
                <CardContent className='p-6'>
                  {upcomingClasses.length > 0 ? (
                    <div className='space-y-4'>
                      {upcomingClasses.map((classItem: any, index: number) => {
                        const batchId = scheduleWithBatchInfo[index]?.batchId || `batch-${index}`;
                        const courseTitle = scheduleWithBatchInfo[index]?.courseTitle || 'Course';
                        return (
                          <div
                            key={batchId}
                          ...
                          >
                            ...
                            <div className='flex items-start justify-between gap-4'>
                              ...
                              <Button
                                ...
                                onClick={() => handleJoinClass(batchId)}
                              >
                        ...
                        >
                          <div className='flex items-start gap-3 mb-3'>
                            ...
                            <div className='flex-1 min-w-0'>
                              ...
                              <p className='text-xs text-gray-600 truncate'>{batch.course.title}</p>
                            </div>
                          </div>
                          <div className='flex items-center justify-between text-xs'>
                            ...
                            <span className='text-gray-500'>by {batch.teacher.name}</span>
                          </div>
                        </div>
                      ))}
                      {batches.length > 3 && (
            ...
            <CardHeader className='border-b border-gray-100 bg-gradient-to-r from-purple-50 to-pink-50'>
              <div className='flex items-center gap-3'>
                ...
                <div>
                  ...
                  <p className='text-sm text-gray-600 mt-1'>Access your resources and tools instantly</p>
                </div>
              </div>
            </CardHeader>
            <CardContent className='p-6'>
              <div className='grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4'>
                ...
                >
                  ...
                  <span className='text-sm font-semibold text-gray-700 text-center'>View Tests</span>
                </button>
                ...
                >
                  ...
                  <span className='text-sm font-semibold text-gray-700 text-center'>Recordings</span>
                </button>
                ...
                >
                  ...
                  <span className='text-sm font-semibold text-gray-700 text-center'>Attendance</span>
                </button>
                ...
                >
                  ...
                  <span className='text-sm font-semibold text-gray-700 text-center'>Performance</span>
                </button>
                ...
                >
                  ...
                  <span className='text-sm font-semibold text-gray-700 text-center'>Notifications</span>
                </button>
          ...
          <div className='grid grid-cols-1 md:grid-cols-3 gap-6'>
            ...
            <Card className='bg-white/90 backdrop-blur-sm border-0 shadow-lg rounded-3xl overflow-hidden'>
              ...
              <CardContent className='p-4'>
                <div className='space-y-3'>
                  {[
                    { icon: CheckCircle, text: 'Completed Math Quiz', time: '2h ago', color: 'green' },
                    { icon: Video, text: 'Joined Physics Class', time: '5h ago', color: 'blue' },
                    { icon: Award, text: 'Earned Achievement', time: '1d ago', color: 'purple' },
                  ].map((item, i) => (
            ...
            <Card className='bg-white/90 backdrop-blur-sm border-0 shadow-lg rounded-3xl overflow-hidden'>
              ...
              <CardContent className='p-4'>
                <div className='grid grid-cols-3 gap-3'>
                  {[
                    { icon: '🏆', label: 'Top Scorer', color: 'yellow' },
                    { icon: '⭐', label: 'Perfect Attendance', color: 'blue' },
                    { icon: '🎯', label: 'Quick Learner', color: 'green' },
                    { icon: '🔥', label: 'Streak Master', color: 'orange' },
                    { icon: '💪', label: 'Hard Worker', color: 'purple' },
                    { icon: '🚀', label: 'Rising Star', color: 'pink' },
                  ].map((badge, i) => (
                    <div
                      key={i}
                    ...
                    >
                      ...
                      <span className='text-xs font-medium text-gray-700 text-center'>{badge.label}</span>
                    </div>
                  ))}
                </div>
              </CardContent>
            ...
            <Card className='bg-white/90 backdrop-blur-sm border-0 shadow-lg rounded-3xl overflow-hidden'>
              ...
              <CardContent className='p-4'>
                <div className='space-y-4'>
                  {[
                    { label: 'Classes Attended', value: 8, max: 10, color: 'blue' },
                    { label: 'Tests Completed', value: 3, max: 4, color: 'purple' },
                    { label: 'Assignments Done', value: 5, max: 5, color: 'green' },
                  ].map((item, i) => (
