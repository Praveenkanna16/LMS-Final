...
const DatabaseViewer: React.FC = () => {
  ...
  const {
    ...
    queryFn: async () => {
      const response = await fetch(
        `${import.meta.env.VITE_API_BASE_URL || 'http://localhost:5001/api'}/admin/data/tables`,
        {
          headers: {
  ...
  const {
    ...
    queryKey: ['admin-table-data', selectedTable, currentPage, searchTerm, sortBy, sortOrder],
    queryFn: async () => {
      if (!selectedTable) return null;
      const response = await fetch(
        `${import.meta.env.VITE_API_BASE_URL || 'http://localhost:5001/api'}/admin/data/tables/${selectedTable}?page=${currentPage}&limit=50&search=${searchTerm}&sortBy=${sortBy}&sortOrder=${sortOrder}`,
        {
          headers: {
            Authorization: `Bearer ${localStorage.getItem('token') || ''}`,
            'Content-Type': 'application/json',
